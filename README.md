# This app is maintained from: **Wav2Lip**: *Accurately Lip-syncing Videos In The Wild* 
...
Please download this weight: Wav2Lip + GAN  | Slightly inferior lip-sync, but better visual quality | [Link](https://iiitaphyd-my.sharepoint.com/:u:/g/personal/radrabha_m_research_iiit_ac_in/EdjI7bZlgApMqsVoEUUXpLsBxqXbn5z8VTmoxp55YNDcIA?e=n9ljGW) and put it on same level with app.py
...
```
Note: Please adjust torch version follow-up your CUDA version.

+ Docker:

You can re-build image by going to docker_common folder and read README.md

Or, you can clone image from **huynhhoanghuy/lipsync:cuda**

Start container with this command:

```bash
bash enter_docker.sh --name {container_name} -v {host_folder_to_mount}:/home/admin/{dest_folder}
```

Example:

```bash
bash enter_docker.sh --name test_1 -v /media/linh/DATA/01.huy.hhoang/wav2lip/:/home/admin/test
cd test
```
## Step by step:

To start app, please run:
```bash
python3.8 app.py
```

Then goto "http://localhost:8000/" on browser.

![Alt text](lipsync_UI.png?raw=true "Lipsync UI")

- Firstly, click 1st "Choose File" button to pick a static image which will be generated by dynamic lipsync.

- Secondly, choose audio:

-- This app has 2 reader-modes: Read wav from file (mode 1) | Read wav from realtime-micro (mode 2).

-- With mode read wav from file, you can choose the 2nd "Choose File" button to choose audio file. Otherwise, you can skip that choose, and please choose "Record audio from micro".

- Thirdly, click "Start lipsync" button to start.

If you are running mode 1, you will wait for getting a "Status: Completed" on UI. Then a link to output will be showed on.
Otherwise, you also wait to get a "Status: Completed" on UI and app will not end, the output image will be showed on UI continuously.
